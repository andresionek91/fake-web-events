...
class TestUser:
    ...
    def test_geo(self, fake_user):
        assert fake_user.geo() == {
    ...
    def test_ip(self, fake_user):
        assert fake_user.ip() == {
            'ip_address': '216.73.11.199'
        }

    def test_browser(self, fake_user):
        assert fake_user.browser() == {
    ...
    def test_operating_system(self, fake_user):
        assert fake_user.operating_system() == {
            'os': 'iPad; CPU iPad OS 12_4 like Mac OS X',
            'os_name': 'iOS',
            'os_timezone': 'America/New_York'}

    def test_device(self, fake_user):
        assert fake_user.device() == {
            'device_type': 'Mobile',
            'device_is_mobile': True
        }

    def test_user(self, fake_user):
        assert fake_user.user() == {
            'user_custom_id': 'nfisher@yahoo.com',
            'user_domain_id': 'd4713d60-c8a7-4639-ab11-67b367a9c378'
        }

    def test_referer(self, fake_user):
        assert fake_user.referer() == {
    ...
    def test_utm(self, fake_user):
        assert fake_user.utm() == {
            'utm_medium': 'organic',
            'utm_source': 'facebook',
            'utm_content': 'ad_3',
...
class TestUserPool:
    ...
    def test_pool_size(self, fake_user_pool):
        assert len(fake_user_pool.pool) == 10

    def test_get_user(self, fake_user_pool):
        assert isinstance(fake_user_pool.get_user(), User)

    def test_pool_size_after_getting_user(self, fake_user_pool):
        fake_user_pool.get_user()
        assert len(fake_user_pool.pool) == 10
